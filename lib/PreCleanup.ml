...
let expand_slice_to_array =
  object (_self)
    ...
    method! visit_expr (((), _) as env) e =
      match e.node with
      | EApp ({ node = ETApp ({ node = EQualified lid; _}, _, _, [_; (TCgApp (TApp (_, [ t ]), cg)) as arr_t; _]); _}, es)
      ...
      | EApp ({ node = ETApp ({ node = EQualified lid; _}, cgs, _, [slice_t; TBuf( (TCgApp (TApp (_, [ t ]), cg)) as arr_t, false) as arr_ref_t; err_t]); _}, [ slice ])
