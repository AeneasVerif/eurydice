...
Eurydice_dst_ref_shared_87 mk_slice_u8(uint8_t *x, size_t len)
{
    Eurydice_dst_ref_shared_87 s;
    s.ptr = x;
    s.meta = len;
    return s;
}
...
TEST(MlKem768TestNeon, NISTKnownAnswerTest)
{
    ...
    for (auto kat : kats)
    {
        ...
        uint8_t sk_hash[32];
        libcrux_sha3_sha256(
            mk_slice_u8(key_pair.sk.value, LIBCRUX_ML_KEM_MLKEM768_SECRET_KEY_SIZE), sk_hash);
